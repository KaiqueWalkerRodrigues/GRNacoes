...
<body class="nav-fixed">
                                        ...
                                        <?php foreach ($Usuario->listarAtivos() as $usuario) { ?>
                                            <tr>
                                                ...
                                                <td class="text-center">
                                                    ...
                                                    <button class="btn btn-datatable btn-icon btn-transparent-dark btn-aviso-ferias" type="button" data-toggle="modal" data-target="#modalAvisoFerias"
                                                        data-nome_empresa="<?php echo Helper::mostrar_empresa2($usuario->empresa) ?>"
                                                        data-cnpj="<?php echo Helper::mostrar_cnpj_empresa($usuario->empresa) ?>"
                                                        data-nomem="<?php echo $usuario->nome ?>"
                                                        data-cpf="<?php echo Helper::formatarCPF($usuario->cpf) ?>"
                                                        data-id_empresa="<?php echo $usuario->empresa ?>"><i class="fa-solid fa-sun"></i></button>
                                                    ...
                                                    <button class="btn btn-datatable btn-icon btn-transparent-dark" type="button" data-toggle="modal" data-target="#modalEditarUsuario"
                                                        ...
                                                        data-ativo="<?php echo $usuario->ativo ?>"><i class="fa-solid fa-gear"></i></button>
                                                    <button class="btn btn-datatable btn-icon btn-transparent-dark" type="button" data-toggle="modal" data-target="#modalDeletarUsuario"
                                                        data-idusuario="<?php echo $usuario->id_usuario ?>"
                                                        data-nome="<?php echo $usuario->nome ?>"><i class="fa-solid fa-trash"></i></button>
                                                </td>
                                            </tr>
                                        <?php } ?>
                                    </tbody>
    ...
    <!-- Modal Aviso de Férias -->
    <div class="modal fade" id="modalAvisoFerias" tabindex="-1" role="dialog" aria-labelledby="modalAvisoFeriasLabel" aria-hidden="true">
        <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Gerar Aviso de Férias</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Fechar">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form id="formAvisoFerias">
                        <div class="form-group">
                            <label>Empresa</label>
                            <input type="text" id="af_empresa" class="form-control" readonly>
                        </div>
                        <div class="form-group">
                            <label>Funcionário</label>
                            <input type="text" id="af_nome" class="form-control" readonly>
                        </div>
                        <div class="form-row">
                            <div class="form-group col-6">
                                <label for="af_inicio">Início do Período</label>
                                <input type="date" id="af_inicio" class="form-control" required>
                            </div>
                            <div class="form-group col-6">
                                <label for="af_fim">Fim do Período</label>
                                <input type="date" id="af_fim" class="form-control" required>
                            </div>
                        </div>
                        <input type="hidden" id="af_cnpj">
                        <input type="hidden" id="af_cpf">
                        <input type="hidden" id="af_id_empresa">
                    </form>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-dark" data-dismiss="modal">Cancelar</button>
                    <button type="button" id="gerarAvisoFeriasBtn" class="btn btn-success">Gerar PDF</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        // Handler para abrir o modal com os dados do usuário
        $(document).on('click', '.btn-aviso-ferias', function() {
            var btn = $(this);
            $('#af_empresa').val(btn.data('nome_empresa'));
            $('#af_nome').val(btn.data('nomem'));
            $('#af_cnpj').val(btn.data('cnpj'));
            $('#af_cpf').val(btn.data('cpf'));
            $('#af_id_empresa').val(btn.data('id_empresa'));

            // Definir datas padrão: início = hoje, fim = +30 dias
            var hoje = new Date();
            var hojeISO = hoje.toISOString().slice(0, 10);
            var futuro = new Date();
            futuro.setDate(futuro.getDate() + 30);
            var futuroISO = futuro.toISOString().slice(0, 10);
            $('#af_inicio').val(hojeISO);
            $('#af_fim').val(futuroISO);
        });

        // Gerar PDF abrindo a URL com os parâmetros
        $('#gerarAvisoFeriasBtn').on('click', function() {
            var inicio = $('#af_inicio').val();
            var fim = $('#af_fim').val();
            if (!inicio || !fim) {
                alert('Preencha início e fim do período.');
                return;
            }
            var params = {
                nome_empresa: encodeURIComponent($('#af_empresa').val()),
                nomem: encodeURIComponent($('#af_nome').val()),
                cnpj: encodeURIComponent($('#af_cnpj').val()),
                cpf: encodeURIComponent($('#af_cpf').val()),
                id_empresa: encodeURIComponent($('#af_id_empresa').val()),
            ...
            var url = '<?php echo URL ?>/configuracoes/docs/aviso_ferias?' + query;
            window.open(url, '_blank');
            $('#modalAvisoFerias').modal('hide');
        });
    </script>
