...
<body class="nav-fixed">
    ...
    <div class="modal fade" id="modalEditarContrato" tabindex="1" role="dialog" aria-labelledby="modalEditarContratoLabel" aria-hidden="true">
        <form action="?" method="post">
            <div class="modal-dialog modal-xl" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            ...
                            <input type="hidden" name="id_financeiro_contrato" id="editar_id_financeiro_contrato">
                            <input type="hidden" name="usuario_logado" value="<?php echo $_SESSION['id_usuario']?> ">
    ...
    <script>
        ...
        $(document).ready(function () {
                        ...
                        success: function (response) {
                            if (response.success) {
                                ...
                                response.parcelas.forEach(parcela => {
                                    ...
                                    }else if(parcela.pago_em == null & new Date(data_original) < new Date()){
            ...
            //Editar
            {
                let isEditarCPFValid = false;
                let isEditarCEPValid = false;

                function atualizarbtnEditar() {
                    if (isEditarCPFValid && isEditarCEPValid) {
                ...
                function formatarCPF(cpf) {
                    ...
                    if (cpf.length === 11) {
                        ...
                        cpf = cpf.replace(/(\d{3})(\d)/, '$1.$2');
                        cpf = cpf.replace(/(\d{3})(\d{1,2})$/, '$1-$2');
                        $('#editar_cpf').val(cpf); // Atualiza o campo com o formato
                    }

                    // Valida o CPF formatado
                    if (validarCPF(cpf)) {
                        $('#editar_cpf').removeClass('is-invalid').addClass('is-valid');
                        isEditarCPFValid = true;
                    } else {
                        $('#editar_cpf').removeClass('is-valid').addClass('is-invalid');
                        isEditarCPFValid = false;
                    }
                }
                ...
                function formatarCEP(cep) {
                    ...
                    if (cep.length === 8) {
                        ...
                        $('#editar_cep').val(cep); // Atualiza o campo com o formato
                        $('#editar_cep').removeClass('is-invalid').addClass('is-valid');
                        isEditarCEPValid = true;
                    } else {
                        $('#editar_cep').removeClass('is-valid').addClass('is-invalid');
                        isEditarCEPValid = false;
                    }
                }
                ...
                $('#editar_cep').on('input', function () {
                    let cep = $(this).val().replace(/\D/g, '');
                    formatarCEP(cep)
                    atualizarbtnEditar();
                });

                // Inicializa o estado do botão ao abrir o modal de edição
                $('#modalEditarContrato').on('shown.bs.modal', function () {
                    let cep = $('#editar_cep').val().replace(/\D/g, '');
                    formatarCEP(cep)
                    let cpf = $('#editar_cpf').val()
                    formatarCPF(cpf)
                ...
                $('#modalEditarContrato').on('show.bs.modal', function (event) {
                    ...
                    $('#editar_id_financeiro_contrato').val(id_financeiro_contrato)
